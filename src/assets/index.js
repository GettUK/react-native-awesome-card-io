/* eslint-disable global-require */
const imagesApp = {
  carTypes: {
    Standard: require('./png/car-types/Standard.png'),
    StandardGett: require('./png/car-types/StandardGett.png'),
    BlackTaxi: require('./png/car-types/BlackTaxiGett.png'),
    OTBlackTaxi: require('./png/car-types/BlackTaxi.png'),
    BlackTaxiOT: require('./png/car-types/BlackTaxi.png'),
    BlackTaxiGett: require('./png/car-types/BlackTaxiGett.png'),
    BlackTaxiXL: require('./png/car-types/BlackTaxiXL.png'),
    OTBlackTaxiXL: require('./png/car-types/MPV.png'),
    Exec: require('./png/car-types/Exec.png'),
    ExecGett: require('./png/car-types/ExecGett.png'),
    MPV: require('./png/car-types/MPV.png'),
    Special: require('./png/car-types/Special.png'),
    GettXL: require('./png/car-types/GettXL.png'),
    GettExpress: require('./png/car-types/GettExpress.png'),
    Economy: require('./png/car-types/Economy.png'),
    StandardXL: require('./png/car-types/StandardXL.png'),
    Business: require('./png/car-types/Business.png')
  },
  noServiceCarTypes: {
    Standard: require('./png/noLogoCarTypes/Standard.png'),
    StandardGett: require('./png/noLogoCarTypes/StandardGett.png'),
    BlackTaxi: require('./png/noLogoCarTypes/BlackTaxiGett.png'),
    BlackTaxiXL: require('./png/noLogoCarTypes/BlackTaxiXL.png'),
    OTBlackTaxiXL: require('./png/noLogoCarTypes/MPV.png'),
    Exec: require('./png/noLogoCarTypes/Exec.png'),
    ExecGett: require('./png/noLogoCarTypes/ExecGett.png'),
    MPV: require('./png/noLogoCarTypes/MPV.png'),
    Special: require('./png/noLogoCarTypes/Special.png'),
    GettXL: require('./png/noLogoCarTypes/GettXL.png'),
    GettExpress: require('./png/noLogoCarTypes/GettExpress.png'),
    Economy: require('./png/noLogoCarTypes/Economy.png'),
    StandardXL: require('./png/noLogoCarTypes/StandardXL.png'),
    Business: require('./png/noLogoCarTypes/Business.png')
  },
  expirationDate: require('./png/application/expirationDate.png'),
  cvv: require('./png/application/cvv.png'),
  clear: require('./png/application/clear-icon.png'),
  loginBg: require('./png/application/login-bg.jpg'),
  loginBgX: require('./png/application/bg_X.png'),
  carShadow: require('./png/car_shadow.png')
};
/* eslint-enable global-require */

export default imagesApp;
